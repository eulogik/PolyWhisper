...
Stage logs: nohup_v4_a1.log, nohup_v4_a2.log, nohup_v4_phase1.log, nohup_v4_phase2.log
...
FLAG = SAVE_DIR / "PIPELINE_V4_DONE.flag"
EN_BEST = SAVE_DIR / "adapters_v3" / "en_best_v4.pt"
PYTHON = str(Path(".venv/bin/python").absolute())
ADAPTERS = SAVE_DIR / "adapters_v3"

STAGES = [
    {"log": SAVE_DIR / "nohup_v4_a1.log", "done": "TRAINING COMPLETE", "timeout_h": 9,
     "cmd": ["train_v3.py", "--langs", "en", "--mask-lang", "en", "--epochs", "3",
             "--lr", "1e-4", "--tag", "_v4"]},
    {"log": SAVE_DIR / "nohup_v4_a2.log", "done": "TRAINING COMPLETE", "timeout_h": 9,
     "cmd": ["train_v3.py", "--langs", "hi", "--encoder-lora", "--epochs", "4",
             "--lr", "1e-4", "--tag", "_v4"]},
    {"log": SAVE_DIR / "nohup_v4_phase1.log", "done": "ROUTER TRAINING COMPLETE", "timeout_h": 8,
     "cmd": ["train_router.py", "--phase", "1", "--epochs", "2", "--tag", "_v4",
             "--en-adapter", str(ADAPTERS / "en_best_v4.pt"),
             "--hi-adapter", str(ADAPTERS / "hi_best_v4.pt")]},
    {"log": SAVE_DIR / "nohup_v4_phase2.log", "done": "ROUTER TRAINING COMPLETE", "timeout_h": 10,
     "cmd": ["train_router.py", "--phase", "2", "--epochs", "2", "--lr", "1e-4",
             "--lambda-router", "1.0", "--tag", "_v4",
             "--router-init", str(ADAPTERS / "router_best_v4.pt"),
             "--en-adapter", str(ADAPTERS / "en_best_v4.pt"),
             "--hi-adapter", str(ADAPTERS / "hi_best_v4.pt")]},
]
