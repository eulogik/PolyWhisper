...
\author{Eulogik~(\url{https://eulogik.com})%
  \thanks{Code: \url{https://github.com/eulogik/PolyWhisper} \quad
          Model: \url{https://huggingface.co/eulogik/polywhisper} \quad
          PyPI: \texttt{pip install polywhisper} \quad Web: \url{https://eulogik.com}}}
...
Rather than full fine-tuning (which requires $\sim$1.5\,GB per language), we train rank-16 LoRA adapters ($\sim$14\,MB each) on the decoder and encoder attention projections, preserving the pretrained acoustic knowledge of the Whisper-Small backbone (244\,M parameters).

We report one novel finding: \textbf{per-language augmentation asymmetry}.
Applying SpecAugment and speed perturbation globally to all languages \emph{damaged} Hindi and Tamil performance (inducing token-loop degeneration) while substantially improving Bengali ($-34.5\%$ relative WER) and Marathi ($-43.2\%$).
...
\noindent\textbf{Keywords:} Indic ASR, Whisper, LoRA, parameter-efficient fine-tuning, multilingual speech, edge deployment
...
\poly\ takes a different approach: \textbf{freeze the backbone, train only the adapters}.
We attach per-language LoRA (Low-Rank Adaptation~\cite{hu2022lora}) modules to the decoder and encoder attention projections of Whisper-Small, training approximately 14\,M parameters per language while keeping the 244\,M-parameter backbone entirely frozen.
Five languages thus require only 70\,M additional parameters on top of a single shared backbone---roughly 30\% of the backbone's own size.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{figures/fig1_architecture.pdf}
  \caption{Architecture. A single frozen Whisper-Small backbone serves all five languages. Per-language LoRA adapters (rank~16, $\sim$14\,MB each) are injected via forward hooks on every attention projection. Switching languages swaps hooks with no weight reloading. ONNX exports fuse the chosen adapter into the graph for CPU inference.}
  \label{fig:arch}
\end{figure}
...
IndicConformer~\cite{chiu2022indicconformer} trained a 600\,M-parameter Conformer on 50\,K+ hours of Indic speech, achieving FLEURS WERs of 14.5 (Hindi), 24.7 (Bengali), 35.2 (Tamil), 28.9 (Telugu), and 21.9 (Marathi).
SraVaani~\cite{javed2024sravaani} further improved to 14.0, 19.8, 26.2, 25.1, and 19.7 using a similar Conformer.
IndicWhisper~\cite{nandyala2025indicwhisper} fine-tuned Whisper Medium on Indic data, reporting 15.0 (Hindi), 20.9 (Bengali), 25.2 (Tamil), 25.4 (Telugu), and 20.5 (Marathi) on FLEURS.
...
Whisper-LoRA~\cite{rybakov2024whisperlora} explored LoRA on Whisper but focused on English.
...
MMS~\cite{pratap2023mms} scaled to 1,100+ languages; USM~\cite{zhang2023usm} to 300+ languages---impressive scaling efforts that remain large GPU-bound models.
Our work complements these by enabling per-language deployment at $\sim$14\,MB per language with INT8 quantization.
...
Each adapter is two low-rank matrices $A\in\mathbb{R}^{d\times r}$ and $B\in\mathbb{R}^{r\times d}$ with $d=512$ and $r=16$.
...
Each language adds $\sim$14\,M trainable parameters ($\sim$14\,MB on disk at FP32).
Five adapters total 70\,M---about $29\%$ of the backbone.

\paragraph{Runtime language switching.}
Languages are switched via PyTorch forward hooks.
$\texttt{set\_language("hi")}$ installs Hindi hooks; switching removes old hooks and installs the target language's hooks with no weight reloading.
...
All evaluation is on the FLEURS benchmark~\cite{conneau2023fleurs} (read speech, Wikipedia sentences).
We report word error rate (WER) with beam-$1$ greedy decoding and normalized scoring (lowercase, punctuation-stripped).
All \poly\ WERs quoted in Tables~\ref{tab:main}--\ref{tab:ablation} and Figures~\ref{fig:wer}--\ref{fig:delta} are the values published in our Hugging Face model card and in \texttt{final\_summary.json} on \texttt{eulogik/polywhisper} (beam-$1$ unless noted); they agree with the raw eval artifacts \texttt{eval\_\{lang\}\_pure\_fleurs*.json} within rounding.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcc}
    ...
    Hindi (hi)  & No  & No  \\
    Tamil (ta)  & No  & No  \\
    Telugu (te) & \multicolumn{2}{c}{Clean in the recommended recipe\footnotemark} \\
    Bengali (bn)& Yes & Yes \\
    Marathi (mr)& Yes & Yes \\
    \bottomrule
  \end{tabular}
\end{table}
\footnotetext{Telugu was held at the clean recipe (no augment) in the final best mix; augmentation variants showed no consistent gain. All v9 rows in Tables~\ref{tab:main}--\ref{tab:ablation} for Telugu are therefore identical under either column.}
...
\begin{itemize}[leftmargin=1.2em, itemsep=2pt]
  ...
  \item \textbf{v9 (selective)}---SpecAugment + speed perturb on \emph{only} Bengali/Marathi; Hindi/Tamil/Telugu clean. The recommended ``final best'' mix picks the per-language best of v9: \texttt{hi\_best\_clean.pt}/\texttt{ta\_best\_clean.pt} for Hindi/Tamil (\S\ref{sec:eval}) and the v9 augmented \texttt{te/bn/mr\_best\_prod.pt} for the others.
\end{itemize}
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    \midrule
    \poly\ v7 (no aug., beam-$1$)            & 244M+5$\times$14M & 43.0 & 68.2 & 105.9 & 198.8 & 170.1 \\
    \poly\ v8 (global aug., beam-$5$)$^\dagger$ & 244M+5$\times$14M & 52.5 & 73.6 & 120.2 & 169.9 & 82.9  \\
    \textbf{\poly\ v9 (sel., beam-$1$)} & \textbf{244M+5$\times$14M} & \textbf{46.3} & \textbf{70.1} & \textbf{100.1} & \textbf{130.2} & \textbf{96.7} \\
    \midrule
    Whisper large-v2~\cite{radford2023robust} & 1.55B & 21.5 & 17.5 & $\sim$46.2 & $\sim$92.6 & 38.3 \\
    IndicConformer~\cite{chiu2022indicconformer} & 600M & 14.5 & 35.2 & 28.9 & 24.7 & 21.9 \\
    IndicWhisper~\cite{nandyala2025indicwhisper} & 769M & 15.0 & 25.2 & 25.4 & 20.9 & 20.5 \\
    SraVaani-1.0~\cite{javed2024sravaani} & $\sim$600M & 14.0 & 26.2 & 25.1 & 19.8 & 19.7 \\
    \bottomrule
  \end{tabular}
  \vspace{1pt}
...
\begin{table}[t]
  \centering
  \caption{Augmentation ablation (FLEURS WER, normalized). Deltas are computed within Table~\ref{tab:main}'s reported numbers; the v8 delta mixes beams ($^\dagger$) and is shown for context.}
  \label{tab:ablation}
  \small
  \begin{tabular}{lccccc}
    ...
    \midrule
    v7 (no aug., beam-$1$)    & 43.0 & 68.2 & 105.9 & 198.8 & 170.1 \\
    v8 (global aug., beam-$5$)$^\dagger$ & 52.5 & 73.6 & 120.2 & 169.9 & 82.9  \\
    v9 (selective, beam-$1$)  & 46.3 & 70.1 & 100.1 & 130.2 & 96.7  \\
    \midrule
    \textbf{v9 $\Delta$ vs v7} & +7.7\% & +2.8\% & \textbf{$-$5.5\%} & \textbf{$-$34.5\%} & \textbf{$-$43.2\%} \\
    v8 $\Delta$ vs v7$^\dagger$ & +21.6\% & +8.0\% & +13.5\% & $-$14.5\% & $-$51.2\% \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{enumerate}[leftmargin=1.2em, itemsep=2pt]
  ...
  \item \textbf{Global augmentation helps Bengali and Marathi.} Bengali $198.8\to169.9$ ($-$14.5\%) and Marathi $170.1\to82.9$ ($-$51.2\%).
  \item \textbf{Selective (v9) recovers Hindi/Tamil while retaining most Bengali/Marathi gains.} Hindi $46.3$ within $7.7\%$ of v7, Tamil $70.1$ within $2.8\%$, while Bengali/Marathi remain $-$34.5\%/$-$43.2\% better than v7.
\end{enumerate}
...
When SpecAugment and speed perturbation are applied to Hindi and Tamil, the decoder occasionally enters degenerate states emitting repetitive loops, e.g.\ Hindi \texttt{``है है है है \ldots''} or Tamil \texttt{``அது அது \ldots''}.
...
\paragraph{Package.}
\texttt{pip install polywhisper} (PyPI: \url{https://pypi.org/project/polywhisper/}) or \texttt{pip install polywhisper[onnx]} for ONNX Runtime.
...
\paragraph{Beam-$1$.}
We report greedy decoding; beam-$5$ typically cuts WER by 10--20\% relative at higher latency. Optimizing beam search is future work.
...
\poly\ provides a complete, reproducible recipe for efficient multilingual Indic ASR: freeze Whisper-Small, train per-language LoRA adapters ($\sim$14\,MB each), and---critically---apply augmentation \emph{per language}, not globally.
The same augmentation that improves Bengali by 34.5\% degrades Hindi by 21.6\%, a divergence that demands language-aware recipes.
We release all code, adapters, eval artifacts, and a PyPI package, plus 20 ONNX graphs on the Hub, to lower the barrier for Indic ASR and to provide a baseline for efficient speech adaptation.
