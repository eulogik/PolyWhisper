...
We attach per-language LoRA (Low-Rank Adaptation~\cite{hu2022lora}) modules to the decoder and encoder attention projections of Whisper-Small, training approximately 3.5\,M parameters per language while keeping the 244\,M-parameter backbone entirely frozen.
Five languages thus require only 17.7\,M additional parameters on top of a single shared backbone---roughly $7\%$ of the backbone's own size.
...
\begin{table}[t]
  ...
  \begin{tabular}{lcccccc}
    ...
    \midrule
    \poly\ v7 (no aug., beam-$1$)$^*$            & 244M+5$\times$3.5M & 43.0 & 68.2 & 105.9 & 198.8 & 170.1 \\
  ...
  \end{tabular}
  \vspace{1pt}
  \caption*{\footnotesize $^\dagger$v8 was evaluated at beam-$5$; all other \poly\ rows are beam-$1$. $^*$v7 baseline for Telugu/Bengali/Marathi was not saved as a structured eval artifact; approximate values pending clean adapter retraining.}
\end{table}
...
\begin{table}[t]
  ...
  \end{tabular}
  \vspace{1pt}
  \caption*{\footnotesize $^\dagger$v8 was evaluated at beam-$5$; all other \poly\ rows are beam-$1$. The v8 vs v7 delta in Table~\ref{tab:ablation} is therefore not strictly apples-to-apples; we include it to show the global-augment effect observed during development. Additionally, the v7 baseline for Telugu/Bengali/Marathi was not saved as a structured eval artifact during development and is approximate; clean adapters for these languages will be retrained and re-evaluated to confirm these numbers.}
\end{table}
...
\end{enumerate}
\vspace{1pt}
{\footnotesize $^*$v7 baseline for Telugu/Bengali/Marathi is approximate (see Table~\ref{tab:main} note).}
...
\poly\ provides a complete, reproducible recipe for efficient multilingual Indic ASR: freeze Whisper-Small, train per-language LoRA adapters ($\sim$14\,MB each on disk), and---critically---apply augmentation \emph{per language}, not globally.
Our preliminary results suggest the same augmentation that improves Bengali by 34.5\% degrades Hindi by 21.6\%, a divergence that demands language-aware recipes; clean baselines for all five languages are being confirmed via retraining.
