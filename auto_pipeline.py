...
def launch_training(lang):
    ...
    if tlog.exists():
        old = SAVE / f"nohup_{lang}.log.prev"
        log(f"{lang}: rotating stale log {tlog.name} -> {old.name} (stale TRAINING COMPLETE would bypass wait)")
        tlog.rename(old)
    cmd = ["caffeinate", "-i", "-s", sys.executable, "train_v3.py",
           "--langs", lang, "--tag", f"_{lang}", "--batch-size", "4", "--epochs", "3"]
    p = subprocess.Popen(cmd, stdout=open(tlog, "a"), stderr=subprocess.STDOUT, env=CHILD_ENV)
...
def evaluate(lang, need_adapter=False):
    fleurs = FLEURS[lang]
    tjson = SAVE / "data" / f"fleurs_{fleurs}_test.json"
    out_vanilla = SAVE / f"eval_vanilla_fleurs_{lang}.json"
    out_pure = SAVE / f"eval_{lang}_pure_fleurs.json"
    if not tjson.exists():
        log(f"{lang}: test json missing ({tjson}); fetching")
        run([sys.executable, "fetch_testset.py", fleurs], timeout=3600)
    if not tjson.exists():
        log(f"{lang}: still no test json — skipping evals")
        return
    if out_vanilla.exists():
        log(f"{lang}: vanilla eval exists ({out_vanilla.name}), skipping")
    else:
        run([sys.executable, "eval_vanilla.py", "--language", lang,
             "--test-json", str(tjson), "--out", str(out_vanilla)], timeout=7200)
    if need_adapter:
        if out_pure.exists():
            log(f"{lang}: pure eval exists ({out_pure.name}), skipping")
        else:
            run([sys.executable, "eval_lang_pure.py", "--lang", lang,
                 "--adapter", f"{lang}_best_{lang}.pt",
                 "--test-json", str(tjson), "--out", str(out_pure)], timeout=7200)
    else:
        log(f"{lang}: no adapter yet — pure eval deferred to post-training")
...
def main():
    ...
    for lang in LANGS:
        ...
        if wait_done(lang, p):
            evaluate(lang, need_adapter=True)
